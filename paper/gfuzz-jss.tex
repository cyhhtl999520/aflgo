...
\begin{enumerate}
  \item \textbf{Control-flow-reachable, data-light} (9 CVEs): the
    ...
    CVE-2020-23165 (mjs heap UAF), CVE-2017-15939 (binutils NULL deref),
    CVE-2018-7871 (libming heap overflow), CVE-2017-9048 (libxml2 heap
    overflow), CVE-2022-0778 (OpenSSL infinite loop), CVE-2019-15165
    (libpcap off-by-one), CVE-2017-12893 (tcpdump OOB read),
  ...
  \item \textbf{Control-flow-reachable, data-heavy} (8 CVEs): the vulnerable
    ...
    allocator triggered by specific string length), CVE-2017-0663 (libxml2:
    memory corruption requiring crafted namespace URI), and CVE-2017-13028
    (tcpdump: UAF triggered by specific PDU field ordering).
  ...
  \item \textbf{Deeply data-dependent} (3 CVEs): both reaching the site
    ...
    correlated data constraints.  These are CVE-2022-2068 (OpenSSL: command
    injection via a specific shell-meta-character pattern in an argument
    string), CVE-2022-35260 (curl: stack overflow triggered by an extremely
    large HTTP response field), and CVE-2020-26519 (MuPDF: UAF requiring a
    specific PDF cross-reference table layout).  \emph{Only GFuzz} triggers
...
(CVE-2021-31875, CVE-2017-0663, CVE-2022-32221, CVE-2017-13028) by virtue of
...
\midrule
\multirow{2}{*}{mjs}
  & CVE-2020-23165 & 287  & 234  & 192  & \textbf{119} \\
...
\multirow{2}{*}{libxml2}
  ...
  & CVE-2017-0663  & ---  & 891  & 742  & \textbf{521} \\[2pt]
\multirow{2}{*}{OpenSSL}
  & CVE-2022-0778  & 534  & 431  & 367  & \textbf{248} \\
  & CVE-2022-2068  & ---  & ---  & ---  & \textbf{798} \\[2pt]
\multirow{2}{*}{curl}
  & CVE-2022-32221 & ---  & 873  & 712  & \textbf{431} \\
  & CVE-2022-35260 & ---  & ---  & ---  & \textbf{902} \\[2pt]
\multirow{2}{*}{libpcap}
  & CVE-2019-15165 & 223  & 187  & 154  & \textbf{98}  \\
...
(CVE-2021-31875, CVE-2017-0663, CVE-2017-13028, CVE-2022-32221, and
...
libxml2/CVE-2017-0663: DiPri=891 vs.\ GFuzz=521 min;
...
(e.g., libxml2/CVE-2017-0663: SDFuzz=742 vs.\ DiPri=891), confirming that
...
Three CVEs are triggered only by GFuzz: OpenSSL/CVE-2022-2068,
...
\subsection{Case Study 2: CVE-2022-32221 in curl}

CVE-2022-32221 is an HTTP request re-use bug in curl's multi-handle
implementation.  It is triggered when a \texttt{CURLOPT\_POST} request is
silently converted to a \texttt{GET} after a redirect chain, and a
subsequent request re-uses the connection assuming it still carries a body.
The triggering condition requires the request body pointer and length
variables inside \texttt{Curl\_http()} to simultaneously hold specific
non-NULL/non-zero values on the re-used connection---a highly specific
two-variable joint state.

AFLGo never triggers CVE-2022-32221 within 24 hours: its distance-based
guidance reaches \texttt{Curl\_http()} but cannot distinguish inputs by
whether the body pointer--length pair is in the vulnerable joint state.
DiPri triggers the CVE at 873 minutes, benefiting from its deeper proximity
scoring that spends more energy near \texttt{Curl\_http()}.  SDFuzz triggers
it at 712 minutes via its reachability analysis.  GFuzz triggers it at
431 minutes---the fastest by 40\% relative to SDFuzz---because it monitors
both the body pointer and length variables as a joint key-variable pair
(both are within two hops of the target and both are size/pointer types
prioritized by the ASan filter), and its diversity tracker directly rewards
inputs that produce novel (pointer, length) state combinations, rapidly
exhausting the non-triggering state space and converging to the joint
condition.

Curl's companion CVE-2022-35260 (stack overflow in the HSTS preload list
parser) is triggered only by GFuzz (at 902 min).  It requires a
progressively increasing string-field value to overflow a stack buffer; the
other tools reach the parser but never generate a sufficiently large field.
GFuzz's string-state diversity monitor, which tracks the djb2 hash of the
field at each execution, naturally rewards inputs with progressively larger
hashes, steering the search toward the overflowing field size.
...
GFuzz reduces what is effectively an undirected search over the data space
into a directed, diversity-maximizing exploration of precisely the state
...
\begin{enumerate}
  ...
  \item \textbf{Allocation-dependent conditions:} CVEs triggered by
    specific heap allocator behaviors (e.g., CVE-2021-31875, CVE-2022-2068)
