...
path-discovery gains appear in binutils (+52.4\% vs.\ AFLGo) and libxml2
(+42.7\%), programs with complex data-dependent code paths.
...
(libpcap, tcpdump, curl), where data-state diversity enables exploration of
...
AFLGo triggers only 9 (45\%), DiPri 13 (65\%), and SDFuzz 16 (80\%).
...
AFLGo, DiPri, and SDFuzz all reach the respective vulnerable
...
than AFLGo, 53.8\% more than DiPri, and 25\% more than SDFuzz.
...
334 min, on those 9 only).  On the 9 CVEs that AFLGo also triggers, GFuzz
achieves a mean TTE of 145 minutes---a \textbf{56.6\%} reduction relative
to AFLGo.  GFuzz consistently delivers the shortest TTE on every individual
CVE it shares with the other tools, because its diversity-driven scheduler
actively explores the data states required to satisfy precise vulnerability
triggering conditions.

\textit{Answer to RQ3:} GFuzz triggers all 20 CVEs and reduces TTE by
56.6\% vs.\ AFLGo on commonly triggered CVEs, while maintaining the fastest
TTE across all DiPri- and SDFuzz-triggered CVEs as well.
...
vs.\ AFLGo, slightly more than SDFuzz (2.9\%) and DiPri (5.7\%), due to
...
AFLGo requires 481 minutes on average and DiPri requires 392 minutes.
...
yielding larger improvements than either alone.
...
GFuzz significantly outperforms AFLGo, DiPri, and SDFuzz: it discovers
50.5\% more execution paths, achieves 30.9\% higher edge coverage, triggers
all 20 CVEs (vs.\ 9/20 for AFLGo), and reduces TTE by 56.6\% on commonly
triggered CVEs---at the cost of only $\sim$9\% runtime overhead.
