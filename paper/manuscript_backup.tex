...
\begin{abstract}
Directed greybox fuzzing (DGF) has emerged as an effective technique for targeted vulnerability detection, focusing computational resources on specific code locations of interest. However, existing DGF approaches primarily rely on control-flow information while neglecting the critical role of program data states in triggering vulnerabilities. Many security bugs require specific combinations of variable values to manifest, which purely coverage-driven approaches may fail to expose efficiently. This paper presents GFuzz, a novel variable state diversity-guided directed fuzzing method that integrates fine-grained data-state awareness into the directed fuzzing process. GFuzz employs a multi-strategy approach to identify key variables related to target locations, instruments programs to monitor their runtime states, and uses state diversity as additional feedback to guide seed selection and energy allocation. An adaptive weight adjustment mechanism dynamically balances coverage-based and state-based guidance throughout the fuzzing campaign. We implemented GFuzz as an extension of AFLGo and evaluated it on four real-world programs (mJS, binutils, libming, and libxml2) containing known vulnerabilities. Experimental results demonstrate that GFuzz achieves significant improvements over AFLGo: 17.4\% increase in path discovery, 11.8\% improvement in code coverage, and 25.9\% enhancement in crash detection, with acceptable 9\% runtime overhead. Our work demonstrates that incorporating variable state diversity into directed fuzzing significantly enhances vulnerability detection effectiveness.
\end{abstract}

\begin{keyword}
Directed fuzzing \sep Greybox fuzzing \sep Variable state diversity \sep Vulnerability detection \sep Program testing
\end{keyword}
