#!/bin/bash
#
# Unibench - uniq Fuzzing with AFLGo/GFuzz
# ----------------------------------------
# 
# This script fuzzes the uniq utility from coreutils using AFLGo
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  Unibench: uniq Fuzzing${NC}"
echo -e "${CYAN}========================================${NC}"
echo
...
# Create targets for uniq - focus on line comparison and field parsing
mkdir -p obj-aflgo/temp
export TMP_DIR=$PWD/obj-aflgo/temp

# Target locations in uniq.c - critical line comparison paths
...
echo -e "${GREEN}✓ Targets specified (line comparison operations)${NC}"
...
# Create seed corpus for uniq - text files with duplicate lines
mkdir -p in
...
echo -e "${GREEN}✓ Seed corpus created (3 text files with duplicates)${NC}"
