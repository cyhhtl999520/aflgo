#!/bin/bash
#
# Unibench - who Fuzzing with AFLGo/GFuzz
# ---------------------------------------
# 
# This script fuzzes the who utility from coreutils using AFLGo
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  Unibench: who Fuzzing${NC}"
echo -e "${CYAN}========================================${NC}"
echo
...
# Create targets for who - focus on utmp file parsing
mkdir -p obj-aflgo/temp
export TMP_DIR=$PWD/obj-aflgo/temp

# Target locations in who.c - critical utmp parsing paths
...
echo -e "${GREEN}✓ Targets specified (utmp parsing operations)${NC}"
...
# Create seed corpus for who - use utmp-style inputs
mkdir -p in

# Create a simple utmp-like file structure
# Note: who can read from files with --file option
echo "user1   tty1         2024-01-01 00:00" > in/seed1.txt
echo "root    pts/0        2024-01-01 12:00" > in/seed2.txt
echo "test    :0           2024-01-01 08:00" > in/seed3.txt

echo -e "${GREEN}✓ Seed corpus created (3 utmp-style seeds)${NC}"
...
echo "  \$AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i in -o out ./obj-aflgo/src/${TARGET_BINARY} --file @@"
echo
...
timeout 60 $AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i in -o out ./obj-aflgo/src/${TARGET_BINARY} --file @@ 2>&1 || true
...
echo "  cd ${WORK_DIR}"
echo "  \$AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i- -o out ./obj-aflgo/src/${TARGET_BINARY} --file @@"
echo
