#!/bin/bash
#
# Unibench - base64 Fuzzing with AFLGo/GFuzz
# ------------------------------------------
# 
# This script fuzzes the base64 encoding/decoding utility from coreutils
# using AFLGo directed greybox fuzzing with optional GFuzz integration.
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  Unibench: base64 Fuzzing${NC}"
echo -e "${CYAN}========================================${NC}"
echo
...
# Create targets for base64 - focus on decode operations which are more vulnerable
mkdir -p obj-aflgo/temp
export TMP_DIR=$PWD/obj-aflgo/temp

# Target locations in base64.c - these are critical decode paths
...
echo -e "${GREEN}✓ Targets specified (decode operations)${NC}"
...
# Create seed corpus for base64
mkdir -p in
echo "SGVsbG8gV29ybGQh" > in/seed1.txt  # "Hello World!" in base64
echo "QUJDREVGRw==" > in/seed2.txt      # "ABCDEFG" in base64
echo "VGVzdA==" > in/seed3.txt          # "Test" in base64
echo "YWJjZDEyMzQ=" > in/seed4.txt      # "abcd1234" in base64

echo -e "${GREEN}✓ Seed corpus created (4 base64-encoded seeds)${NC}"
...
echo "  \$AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i in -o out ./obj-aflgo/src/${TARGET_BINARY} -d @@"
echo
...
timeout 60 $AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i in -o out ./obj-aflgo/src/${TARGET_BINARY} -d @@ 2>&1 || true
...
echo "  cd ${WORK_DIR}"
echo "  \$AFLGO/afl-2.57b/afl-fuzz -m none -z exp -c 45m -i- -o out ./obj-aflgo/src/${TARGET_BINARY} -d @@"
echo
